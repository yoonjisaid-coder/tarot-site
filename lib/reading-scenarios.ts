...
['breakup','ex-contact','헤어진 상대에게 연락이 올까?','Will my ex contact me?','마지막 대화의 여운|연락을 막는 거리|연락 앞에서 나의 선택','침묵에는 정리할 시간, 갈등 회피, 관계를 끝내려는 의사 등 서로 다른 이유가 있을 수 있어요. 카드의 분위기만으로 연락 날짜를 정할 수는 없어요.','연락이 왔을 때 듣고 싶은 말보다 먼저 확인해야 할 변화가 무엇인지 정해 보세요.'],
...
['crush','thinking-of-me','상대의 일상에 내가 떠오를까?','Am I on their mind?','기억에 남는 교류|관심을 표현하는 방식|자연스럽게 연결될 계기','기억에 남는 것과 연애를 원하는 것은 같은 뜻이 아니에요. 작은 취향이나 나눈 이야기를 기억하고 다시 꺼내는 행동을 단서로 볼 수 있어요.','둘이 나누었던 주제 하나로 대화를 열고, 상대도 내 이야기를 궁금해하는지 느껴 보세요.'],
['crush','confess','지금 마음을 고백해도 괜찮을까?','Am I ready to tell them?','내 마음의 준비|고백을 받아들일 관계의 여유|전할 말과 속도','고백은 관계를 보장받는 시험보다 내 마음을 정직하게 알리는 선택에 가까워요. 어떤 답이 와도 상대와 나의 선택권이 남아 있어야 해요.','마음은 분명하게 전하되 바로 답하지 않아도 된다는 여유를 줄 수 있는지 생각해 보세요.'],
...
['undefined','define-us','우리는 연인이 될 수 있을까?','Can this become a relationship?','지금 우린 어디쯤일까|관계를 정하기 어려운 이유|함께 선택할 다음 단계','함께 즐거운 시간을 보내는 것과 서로를 연인으로 선택하는 것은 별도의 합의가 필요해요.','내가 원하는 만남의 형태를 말하고 상대가 원하는 형태도 물어보세요. 모호한 답이 반복되면 그것도 현재의 정보예요.'],
...
['single','new-connection','새 인연을 위해 무엇을 열어볼까?','Where can I make room for connection?','이미 가진 매력과 자원|만남을 좁히는 습관|열어볼 새로운 경험','정해진 누군가가 도착할 날을 기다리기보다, 내가 자연스럽게 나다울 수 있는 만남의 환경을 살펴봐요.','관심 있는 활동 하나를 골라 보세요. 연애 성과보다 내가 즐길 수 있는 자리를 고르는 것이 시작이에요.'],
['single','healthy-love','나에게 잘 맞는 사랑은 어떤 모습일까?','What kind of love suits me?','내가 원하는 정서적 연결|편안함을 만드는 현실 조건|놓치지 말아야 할 기준','이 질문은 미래 상대의 신상이나 외모를 맞히는 대신, 나에게 맞는 관계의 성질을 살펴봐요.','설렘 외에도 대화 방식, 혼자만의 시간, 갈등을 푸는 태도 중 꼭 필요한 기준을 골라 보세요.']
];
...
const positionNotes=[
'첫 번째 카드는 질문의 바탕을 읽는 자리예요. 이미 있었던 대화나 내가 되풀이해서 떠올리는 장면 중 이 상징과 닿는 부분을 찾아보세요. 여기서 느껴지는 감정은 출발점이지, 마지막 카드의 결론까지 미리 정하는 답은 아니에요.',
'두 번째 카드는 첫인상만으로 보이지 않는 조건을 읽어요. 좋은 감정이 있어도 표현이 어려울 수 있고, 불편함이 있어도 익숙해서 계속 이어질 수 있어요. 앞 카드의 이야기가 현실에서 어떤 지점에 막히거나 보완되는지를 살펴보는 자리예요.',
'세 번째 카드는 앞의 두 흐름을 확인한 뒤 선택할 방향이에요. 현재와 같은 반응이 이어진다면 어떤 경험이 반복될지, 작은 선택을 바꾸면 무엇이 달라질 수 있을지 함께 생각해 보세요. 미래는 내 행동뿐 아니라 상대의 선택과 상황에도 영향을 받아요.'
];
const suitNotes:Record<string,string>={
'Major Arcana':'이 메이저 카드는 순간적인 연락 하나보다 관계를 바라보는 기준이나 마음의 전환처럼 큰 주제에 무게를 둬요. 지금의 감정을 급히 결론짓기보다 내가 어떤 관계 안에서 나답게 지낼 수 있는지를 함께 보세요.',
Wands:'완드는 끌림과 행동의 에너지를 보여줘요. 먼저 대화를 열거나 약속을 제안하는 힘이 어디에 있는지 살펴보세요. 시작하는 열정과 이후에도 계속 관심을 기울이는 힘은 서로 다를 수 있어요.',
Cups:'컵은 감정을 받아들이고 나누는 방식에 초점을 맞춰요. 기분 좋은 순간뿐 아니라 약한 모습을 드러냈을 때도 이야기를 들어주는지, 마음을 표현한 뒤 편안함이 남는지를 함께 살펴보세요.',
Swords:'소드는 생각과 말, 판단이 관계에 미치는 영향을 보여줘요. 실제로 들은 문장과 내가 그 말에 덧붙인 해석을 나누어 보세요. 머릿속에서 답을 완성하기보다 확인할 질문 하나를 찾는 데 도움이 되는 카드예요.',
Pentacles:'펜타클은 시간과 일상, 반복되는 행동을 통해 관계를 읽어요. 큰 감정 표현보다 약속한 시간을 지키는지, 서로의 생활을 존중하는지에서 이 주제가 드러날 수 있어요. 느린 흐름과 아무 변화 없는 상태도 구분해 보세요.'};
export function detailedReading(d:Draw,q:Scenario,i:number,ko:boolean){const base=d.reversed?d.card.reversed:d.card.upright;const s=situations.find(s=>s.id===q.situation);if(!ko)return [{title:q.positions[i],text:`${d.card.name} — ${d.card.keyword}. ${base} Read this specifically in the position of “${q.positions[i]}”, in relation to “${q.title}”.`},{title:'In your situation',text:q.focus},{title:'A second perspective',text:d.reversed?'The reversed position asks where this quality is blocked, excessive, or being reconsidered. It does not automatically mean the opposite or a negative outcome.':'The upright position makes this theme more direct, but it remains a perspective to compare with the relationship you are experiencing.'},{title:'A step to consider',text:q.prompt}];return [{title:q.positions[i],text:`${d.card.name}${d.reversed?' 역방향':' 정방향'}의 중심은 ‘${d.card.keyword}’이에요. ${base} ${questionLens(d,q.id)}`},{title:s?`${s.ko} 상황에서 읽으면`:'이 질문에서 읽으면',text:`${q.situation==='single'?s?.context:cardScene(d.card.id)} ${positionNotes[Math.min(i,2)]}`},{title:'더 살펴볼 단서',text:`${suitNotes[d.card.suit]} ${d.reversed?'역방향에서는 이 카드의 성질이 충분히 표현되지 않거나 지나치게 작용하는 부분을 함께 살펴봐요. 무조건 반대의 결론이라는 뜻은 아니에요.':'정방향은 이 카드의 주제가 비교적 직접적으로 드러나는 방식으로 읽어요. 다만 한 장만으로 상대가 같은 마음이라고 결론내리지는 않아요.'}`},{title:i===2?'지금 해볼 수 있는 선택':i===1?'나에게 물어볼 질문':'내 경험과 연결하기',text:i===2?q.prompt:i===1?`‘${q.positions[i]}’을 생각할 때 ${d.card.keyword}이라는 주제가 드러나는 구체적인 장면이 있나요? ${q.focus}`:`최근의 교류에서 ‘${d.card.keyword}’과 닮은 순간 하나를 떠올려 보세요. 그때 실제로 오간 말과 행동을 적고, 그 위에 덧붙인 내 기대나 걱정을 따로 구분해 보세요.`}]}
...
export function spreadConnection(ds:Draw[],q:Scenario,ko:boolean){if(ds.length<3)return '';const [a,b,c]=ds;const tense=(d:Draw)=>d.reversed||d.card.tone<0;const same=tense(a)===tense(b);return ko?`‘${q.positions[0]}’의 ${a.card.name}와 ‘${q.positions[1]}’의 ${b.card.name}를 함께 보면, ${same?'비슷한 정서가 이어지면서 그 흐름을 한 번 더 확인하게 돼요.':'첫인상과 그 아래의 조건이 다르게 나타나요. 한 장을 다른 장의 취소로 읽기보다 두 모습이 함께 존재할 수 있다는 점을 살펴보세요.'} 마지막 ${c.card.name}는 ‘${c.card.keyword}’을 통해 다음 선택을 생각하게 해요. ${c.reversed?'이 성질이 막히거나 과해지는 순간을 알아차리는 것부터 시작할 수 있어요.':'이 성질을 내 실제 행동에 작게 옮겨 보는 것이 출발점이 될 수 있어요.'}`:`${a.card.name} in “${q.positions[0]}” meets ${b.card.name} in “${q.positions[1]}”. ${same?'The two positions reinforce a related emotional pattern.':'The visible experience and its underlying conditions point in different directions; allow room for both.'} ${c.card.name} brings “${c.card.keyword}” into your next step. ${q.prompt}`}
