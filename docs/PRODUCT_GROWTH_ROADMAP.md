...
- 기존 타로/운세 사이트의 UX·콘텐츠·수익화 방향을 유지하면서 반복적인 콘텐츠/SEO/QC 업무를 중앙관제실에 연결한다. 이 문서의 기존 확정사항과 보류 결정이 우선한다.
- 분석 흐름은 `Search demand → Problem/intent → free entry content → reading/product UX → SEO → conversion → performance → improvement`로 한다. 실행 시에는 위 Human Approval 및 QC 절차를 함께 적용한다.
- “타로 3장” 같은 기능 자체보다 사용자가 알고 싶은 문제와 답을 중심으로 설계한다. 카드 수·스프레드는 질문을 풀기 위한 수단으로 설명하며, 기존 수동 카드 선택과 덱 구성은 유지한다.
- 모호한 번역체 문구를 피하고, 질문에 대한 답과 카드 근거를 쉬운 일상 언어로 연결한다.
- 독자에게 고유한 답이나 실질적인 도움이 없는 thin SEO page를 만들지 않는다. 검색어만 바꾼 유사 페이지의 대량 생성보다 검색의도별 유용성과 기존 페이지 개선을 우선한다.
